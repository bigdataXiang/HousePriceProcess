...
public class AdminAddr {
	 static String Folder="D:/zhouxiang/人口数据/区划数据/test/IDAddr/exception.txt";
	 ...
	 public static void processCSV(String file) throws IOException{
			...
			for (int n = 0; n < pois.size(); n ++) {
				if (batch) {
					...
					String[] tempoi=rs.split(",");
					String addr0=tempoi[1];
					String peopoi="<AdminCode>"+tempoi[0].substring(0, 6)+"</AdminCode>"+"<AdminAddr>"+addr0+"</AdminAddr>";
					validpois.add(peopoi);
					count ++;
												...
												if (gq != null && gq.getResult() != null && gq.getResult().size() > 0)
												{
													...
													for (int m = 0; m < gq.getResult().size(); m ++)
													{
														if (gq.getResult().get(m) != null && gq.getResult().get(m).getLocation() != null)
														{
															...
															Admin="<Hukou_Admin>"+gq.getResult().get(m).getLocation().getRegion().getProvince()+","+gq.getResult().get(m).getLocation().getRegion().getCity()+","+gq.getResult().get(m).getLocation().getRegion().getCounty()+","+gq.getResult().get(m).getLocation().getRegion().getTown()+"</Hukou_Admin>";
															else
																Admin="暂无";
															//System.out.println(Admin);
															lnglat = "<Admin_Coor>" + gq.getResult().get(m).getLocation().getLng() + ";" + gq.getResult().get(m).getLocation().getLat()+"</Admin_Coor>";
					...
					try {
						...
						if(el.isJsonObject())
						{
							...
							if (gq != null && gq.getResult() != null && gq.getResult().size() > 0 && gq.getResult().get(0).getLocation() != null)
							{
								...
								admin="<Hukou_Admin>"+gq.getResult().get(0).getLocation().getRegion().getProvince()+","+gq.getResult().get(0).getLocation().getRegion().getCity()+","+gq.getResult().get(0).getLocation().getRegion().getCounty()+","+gq.getResult().get(0).getLocation().getRegion().getTown()+"</Hukou_Admin>";
								else
									admin="暂无";
								lnglat ="<Admin_Coor>"+gq.getResult().get(0).getLocation().getLng() + ";" + gq.getResult().get(0).getLocation().getLat()+ "</Admin_Coor>" ;
