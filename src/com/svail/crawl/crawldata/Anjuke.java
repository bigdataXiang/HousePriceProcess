...
public static void main(String[] args) {
		...
		{getRentOutInfo(regions[i]);} //11.19改
		
		getRentalInfo();//11.19改
		
		//getNewBuildingInfo(regions[0]);
    /*
	   for(int i=4;i<regions.length;i++)
	   {
...
//						}	
			...
			parser.reset();
				...
				if(nodes!=null&&nodes.size()==1)
				{
					...
					if(ix!=-1&&ix2!=-1)
					{
						String sub=str.substring(ix+"楼盘户型".length(), ix2).replace("全部户型", "").replace(" ", "").trim();
						if(sub!=null&&sub.indexOf("暂无") == -1&&sub.indexOf("待定")==-1)
							poi+="<TYPE>"+sub+"</TYPE>";
					}
...
//						}
			...
			if (nodes != null && nodes.size() == 1)
			{
                ...
                //代码的问题出在哪里?
				filter =new AndFilter((new TagNameFilter("li")),new HasParentFilter(new AndFilter(new TagNameFilter("ul"), new HasAttributeFilter("class", "info"))));//new AndFilter((new TagNameFilter("li"),new HasParentFilter(new AndFilter(new TagNameFilter("ul"), new HasAttributeFilter("class", "info")))
				nodes = parser.extractAllNodesThatMatch(filter);
				if (nodes != null)
				{
					for (int mm = 0; mm < nodes.size(); mm ++)
																...
																if (m != -1 && k != -1)
																{
																	...
																	else
																	{
																		...
																		//synchronized是Java语言的关键字，当它用来修饰一个方法或者一个代码块的时候，能够保证在同一时刻最多只有一个线程执行该段代码。
																		{
																			...
																			FileTool.Dump(poi, FOLDER+"/bj_RentoutInfo.txt", "UTF-8");
																		}
																	}
																}
															}
							...
							if (m != -1 && k != -1)
							{
								...
								else
								{
									...
									//synchronized是Java语言的关键字，当它用来修饰一个方法或者一个代码块的时候，能够保证在同一时刻最多只有一个线程执行该段代码。
									{
										...
										FileTool.Dump(poi, "D:\\求租_安居客_北京.csv", "UTF-8");
									}
								}
							}
						}
