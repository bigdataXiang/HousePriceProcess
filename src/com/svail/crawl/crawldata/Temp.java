...
public class Temp{
	...
	Temp(String line) throws IOException
	{
		try{
			...
			else
			{
			 ...
			 PRICE= getStrByKey(line,"<PRICE>","</PRICE>").replace("[面议]","").replace("[押一付三]","").replace("万","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 if(line.indexOf("<DOWN_PAYMENT>")!=-1)
			 DOWN_PAYMENT=getStrByKey(line,"<DOWN_PAYMENT>","</DOWN_PAYMENT>").replace("[面议]","").replace("[押一付三]","").replace("万","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 if(line.indexOf("<UNIT_PRICE>")!=-1)
			 UNIT_PRICE=getStrByKey(line,"<UNIT_PRICE>","</UNIT_PRICE>").replace("元/平米","").replace("[押一付三]","").replace("万","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 if(line.indexOf("<BUILDING_NAME>")!=-1)
			 COMMUNITY=getStrByKey(line,"<BUILDING_NAME>","</BUILDING_NAME>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 ...
			 if(line.indexOf("<BUILDING_AREA>")!=-1)
			 AREA=getStrByKey(line,"<BUILDING_AREA>","</BUILDING_AREA>").replace("㎡","").replace("平方米","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","").replace("�","");
			 ...
			 if(line.indexOf("<BUILDING_DIR>")!=-1)
			 {
				 
				 ORIENTATION=getStrByKey(line,"<BUILDING_DIR>","</BUILDING_DIR>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";",""); 
			 ...
			 if(line.indexOf("<BUILDING_FLOOR>")!=-1)
			 {
			    FLOOR=getStrByKey(line,"<BUILDING_FLOOR>","</BUILDING_FLOOR>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			    if(FLOOR.indexOf("(")!=-1)
			   {
			    // FLOOR=FLOOR.replace("/","//");
			     infloor=FLOOR.substring(FLOOR.indexOf("第"), FLOOR.indexOf("层")).replace("第", "").replace("层", "");
			     totalfloor=FLOOR.substring(FLOOR.indexOf("共")).replace("共", "").replace("层", "").replace(")", "");
			   }
			 }
			 if(line.indexOf("BUILDING_CONDITION")!=-1)
			 {
				 FITMENT=getStrByKey(line,"<BUILDING_CONDITION>","</BUILDING_CONDITION>").replace("　","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			...
			if(line.indexOf("<BUILDING_TYPE>")!=-1)
			PROPERTY_TYPE=getStrByKey(line,"<BUILDING_TYPE>","</BUILDING_TYPE>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");	
	...
	public static void main(String[] args) throws IOException {
		String POI="";
		Vector<String> rds = FileTool.Load("D:/Data_classify/finish_classify/anjuke/anjuke_result/anjuke_resold/anjuke_resold0802_0808.txt","UTF-8");
		...
		for (int n = 0; n < rds.size(); n ++)
		{
			...
			write_append(POI,"D:/Crawl_test/anjuke_resold0802_0808.txt");	
			POI="";
		}
	}
