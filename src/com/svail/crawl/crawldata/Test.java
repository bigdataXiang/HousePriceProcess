...
//						{
//
//							String sub = tt.substring(ix + "参考月供".length()).replace("\r\n", "").replace("\t", "").replace(" ", "").trim();
//							if (sub!=""&&sub.indexOf("暂无") == -1)
//								poi += "<MONTHLY_PAYMENT>" + sub.replace(" ", "") + "</MONTHLY_PAYMENT>";
//							continue;
//						}
...
//						}
   ...
   * */
	public static String regions[] = {
			"/chaoyang/","/haidian/","/dongchenga/","/xicheng/",
			"/chongwen/","/xuanwu/","/fengtai/","/tongzhou/",
			"/shijingshan/","/fangshan/","/changping/","/daxing/",
			"/shunyi/","/miyun/","/huairou/","/yanqing/",
			"/pinggua/","/mentougou/","/yanjiao/","/zhoubiana/",
								...
								if (m != -1 && k != -1)
								{
									...
									else
									{
										...
										//synchronized是Java语言的关键字，当它用来修饰一个方法或者一个代码块的时候，能够保证在同一时刻最多只有一个线程执行该段代码。
										{
											...
											FileTool.Dump(poi, "D:\\二手房_安居客_北京.csv", "UTF-8");
										}
									}
								}
							}
								...
								if (m != -1 && k != -1)
								{
									...
									else
									{
										...
										//synchronized是Java语言的关键字，当它用来修饰一个方法或者一个代码块的时候，能够保证在同一时刻最多只有一个线程执行该段代码。
										{
											...
											FileTool.Dump(poi, "D:\\出租_安居客_北京.csv", "UTF-8");
										}
									}
								}
							}
