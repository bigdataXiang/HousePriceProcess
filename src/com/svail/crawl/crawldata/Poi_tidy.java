...
public class Poi_tidy {
	...
	Poi_tidy(String line) throws IOException
	{
		try{
			...
			else
			{
			    ...
			    write_append(line,"D:/Crawldata_beijing/58_problempoi/58_rentout_problem_poi.txt");
			}
			 ...
			 PRICE= getStrByKey(line,"<PRICE>","</PRICE>").replace("元/月","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 if(line.indexOf("<DISTRICT>")!=-1)
			 COMMUNITY=getStrByKey(line,"<DISTRICT>","</DISTRICT>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 ...
			 if(line.indexOf("GENERAL_SITUATION>")!=-1)
			 HOUSE_TYPE=getStrByKey(line,"<GENERAL_SITUATION>","</GENERAL_SITUATION>").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 ...
			 {
			   AREA=HOUSE_TYPE.substring(HOUSE_TYPE.indexOf("卫")+"卫".length(), HOUSE_TYPE.indexOf("㎡")).replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
			 }
			 if(HOUSE_TYPE.indexOf("朝向")!=-1)
			 {
				 
				 ORIENTATION=HOUSE_TYPE.substring(HOUSE_TYPE.indexOf("朝向")+"朝向".length()).replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";",""); 
				 if(ORIENTATION.indexOf("　")!=-1&&(ORIENTATION.indexOf("　")>ORIENTATION.indexOf("朝向")))
			...
			if(FLOOR!=null&&FLOOR.indexOf("/")!=-1)
			 ...
			 {
				 FITMENT=HOUSE_TYPE.substring(HOUSE_TYPE.indexOf("住宅")+"住宅".length(), HOUSE_TYPE.indexOf("朝向")).replace("　","").replace("&nbsp;", "").replace("&amp;","").replace("nb;","").replace("nbsp","").replace("|","").replace(";","");
				if(FITMENT.equals("毛坯"))
				{
					roughcast=0;
				}
				 if(FITMENT.equals("简单装修"))
				 {
					 Simple_decoration=1;
				 }
				 if(FITMENT.equals("中等装修"))
				 {
	...
	public static void main(String[] args) throws IOException {
		String POI="";
		Vector<String> rds = FileTool.Load("D:/Data_classify/finish_classify/58/58_result/58_rentout/58_rentout0405_0411.txt","UTF-8");
		//D:/Data_classify/finish_classify/58/58_result/58_rentout/58_rentout0712_0718.txt
		
		for (int n = 3; n < rds.size(); n ++)
		{
			String element=rds.elementAt(n);
			Poi_tidy poi = new Poi_tidy(element);
			...
			write_append(POI,"D:/Crawl_test/58_rentout0405_0411.txt");	
			POI="";
		}
	}
